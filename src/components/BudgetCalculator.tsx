...
const createEmptyQuote = (): QuoteInput => ({
  ...DEFAULT_QUOTE,
  projectName: "",
  executionHours: 0,
  meetingHours: 0,
      ...
      <span className="compact-input">
        ...
        <input
          ...
          onChange={(event) => {
            ...
            onChange(Math.min(upper, Math.max(0, parsed)));
          }}
        />
...
export function BudgetCalculator() {
  const [input, setInput] = useState<QuoteInput>(() => createEmptyQuote());
  ...
  const choose = <K extends keyof SelectionState>(key: K, value: QuoteInput[K]) => {
    set(key as keyof QuoteInput, value as QuoteInput[keyof QuoteInput]);
  ...
  const missing = useMemo(() => {
    ...
    if (input.billableHours <= 0 || input.monthlyTarget + input.monthlyCosts <= 0) items.push("sua base de preço");
    return items;
  }, [pricingMode, selected, input.executionHours, input.revisions, input.billableHours, input.monthlyTarget, input.monthlyCosts]);

  const isReady = missing.length === 0;
  const showMoney = (value: number) => (isReady ? currency.format(value) : "—");
  const showMetric = (value: number, suffix = "") => (isReady ? `${number.format(value)}${suffix}` : "—");

  const summary = useMemo(() => {
    if (!isReady || !pricingMode) return "Complete as opções do orçamento antes de gerar o resumo.";
    return [
  ...
  }, [input, result, pricingMode, isReady]);
  ...
  const saveQuote = () => {
    ...
    try {
      ...
      // O orçamento continua funcionando sem armazenamento local.
    }
    setSaved(true);
  ...
  const saveProfile = () => {
    try {
      ...
      // Mantém os valores da sessão mesmo sem storage.
    }
  ...
  const loadSaved = (item: SavedQuote) => {
    ...
    setSelected({ projectType: true, workModel: true, complexity: true, scope: true, urgency: true });
    ...
    <div className="view-stack">
      ...
      <div className="control-block pricing-mode-block">
        <div className="control-heading">
          ...
          <span>{pricingMode ? pricingModeDescriptions[pricingMode] : "Escolha como você quer posicionar este orçamento."}</span>
        </div>
      ...
      <div className="control-block">
        <div className="control-heading">
          ...
          <span>{selected.projectType ? projectDescriptions[input.projectType] : "Selecione o tipo desta demanda."}</span>
        </div>
        <ChoiceGroup<ProjectType>
          value={selected.projectType ? input.projectType : null}
          onChange={(value) => choose("projectType", value)}
          options={(Object.keys(labels.projectType) as ProjectType[]).map((value) => ({ value, label: labels.projectType[value] }))}
          columns={3}
        />
      </div>
      ...
      <div className="control-block">
        <div className="control-heading">
          ...
          <span>{selected.workModel ? workDescriptions[input.workModel] : "Selecione a relação comercial deste trabalho."}</span>
        </div>
        <ChoiceGroup<WorkModel>
          value={selected.workModel ? input.workModel : null}
          onChange={(value) => choose("workModel", value)}
          options={(Object.keys(labels.workModel) as WorkModel[]).map((value) => ({ value, label: labels.workModel[value] }))}
          columns={2}
        />
      </div>
    </div>
  ...
  const renderTimeView = () => (
    <div className="view-stack">
      <div className="view-summary">
        <div><span>tempo informado</span><strong>{input.executionHours || input.meetingHours || input.supportHours ? `${number.format(result.totalHours)}h` : "—"}</strong></div>
      ...
      <div className="field-grid">
        ...
        <NumberField label="Revisões" hint={input.revisions > 0 ? `${number.format(result.revisionHours)}h de buffer calculado.` : "Informe quantas rodadas serão incluídas."} value={input.revisions} suffix="x" min={1} max={8} onChange={(value) => set("revisions", Math.round(value))} />
      </div>
    </div>
  );
  ...
  const renderContextView = () => (
    <div className="view-stack compact-groups">
      <div className="control-block">
        <div className="control-heading"><strong>Complexidade</strong><span>Escolha o nível técnico da demanda.</span></div>
        <ChoiceGroup<Level>
      ...
      <div className="control-block">
        <div className="control-heading"><strong>Escopo</strong><span>Escolha o quanto o escopo está definido.</span></div>
      ...
      <div className="control-block">
        <div className="control-heading"><strong>Urgência</strong><span>Escolha o prazo real combinado com o cliente.</span></div>
        <ChoiceGroup<Urgency>
  ...
  const renderFinanceView = () => (
    <div className="view-stack">
      <div className="field-grid finance-grid">
        ...
        <NumberField label="Desconto" hint={isReady ? `Limite seguro agora: ~${maxDiscount}%` : "Preencha o restante para calcular o limite."} value={input.discount} suffix="%" max={60} onChange={(value) => set("discount", value)} />
      </div>
      <label className="travel-toggle">
        <span className="travel-toggle-copy"><MapPin size={18} /><span><strong>Deslocamento</strong><small>Ative somente quando houver saída presencial.</small></span></span>
      ...
      <div className={`health-line ${isReady && safeDiscount ? "is-safe" : "is-risk"}`}>
        ...
        <span>{isReady ? (safeDiscount ? "Preço protegido com as escolhas atuais." : "O desconto atual invade o preço mínimo calculado.") : `Faltam ${missing.length} item(ns) para fechar o cálculo.`}</span>
      </div>
    </div>
  );
  ...
  return (
    <main className="mond-tool">
      <header className="tool-header">
        <div className="brand-lockup" aria-label="Mond Orçamentos"><span className="brand-glyph">m</span><span className="brand-name">mond</span><span className="brand-divider">/</span><span className="brand-product">orçamentos</span></div>
        <div className="header-actions">
          <button type="button" className="header-chip" onClick={() => setProfileOpen(true)}><Gauge size={16} /><span>{result.baseHourlyRate > 0 ? `${currency.format(result.baseHourlyRate)}/h` : "Minha hora"}</span></button>
      ...
      <div className="workspace">
        <aside className="profile-panel">
          ...
          <div className="profile-rate"><span>sua hora-base</span><strong>{result.baseHourlyRate > 0 ? currency.format(result.baseHourlyRate) : "—"}</strong><small>por hora faturável</small></div>
          <dl className="profile-list">
            <div><dt>Meta mensal</dt><dd>{input.monthlyTarget > 0 ? currency.format(input.monthlyTarget) : "—"}</dd></div>
            <div><dt>Custos fixos</dt><dd>{input.monthlyCosts > 0 ? currency.format(input.monthlyCosts) : "—"}</dd></div>
            <div><dt>Horas vendáveis</dt><dd>{input.billableHours > 0 ? `${number.format(input.billableHours)}h` : "—"}</dd></div>
          </dl>
          <button type="button" className="profile-edit" onClick={() => setProfileOpen(true)}>Ajustar minha base</button>
          <div className="profile-insights">
            <span>leitura rápida</span>
            <div><small>Modo</small><strong>{pricingMode ? labels.pricingMode[pricingMode] : "—"}</strong></div>
            <div><small>Horas</small><strong>{input.executionHours > 0 ? `${number.format(result.totalHours)}h` : "—"}</strong></div>
            <div><small>Status</small><strong>{isReady ? "Pronto" : `${missing.length} falta(m)`}</strong></div>
          </div>
        </aside>

        <section className="editor-panel">
          <div className="editor-top">
            <div><span className="editor-kicker">orçamento atual</span><h1>{input.projectName || "Novo orçamento"}</h1></div>
        ...
        <aside className="result-panel" aria-live="polite">
          <div className="result-topline"><span>valor para cobrar</span><span className="status-pill">{isReady ? "atualizado" : "aguardando"}</span></div>
          <div className="result-price"><strong>{showMoney(result.finalPrice)}</strong><span>{isReady && pricingMode ? `${labels.pricingMode[pricingMode]} · ${input.discount > 0 ? `${input.discount}% de desconto aplicado` : "preço recomendado"}` : "preencha as opções para calcular"}</span></div>
          <div className="price-bounds">
            <div><span>piso</span><strong>{showMoney(result.floorPrice)}</strong></div>
            <div><span>recomendado</span><strong>{showMoney(result.recommendedPrice)}</strong></div>
            <div><span>premium</span><strong>{showMoney(result.premiumPrice)}</strong></div>
          </div>
          <div className="result-metrics">
            <div><span>hora comercial</span><strong>{showMoney(result.pricingHourlyRate)}</strong></div>
            <div><span>sobra estimada</span><strong>{showMoney(result.expectedNet)}</strong></div>
            <div><span>margem final</span><strong>{showMetric(result.expectedNetPercent, "%")}</strong></div>
            <div><span>horas totais</span><strong>{showMetric(result.totalHours, "h")}</strong></div>
          </div>
          <div className="payment-row">
            <div><span>entrada</span><strong>{showMoney(result.entryPrice)}</strong></div>
            <div><span>entrega</span><strong>{showMoney(result.finalPayment)}</strong></div>
          </div>
          {!isReady ? (
            <div className="warning-box"><AlertTriangle size={16} /><span>Complete: {missing.slice(0, 3).join(", ")}{missing.length > 3 ? ` +${missing.length - 3}` : ""}.</span></div>
          ...
          <div className="result-actions">
            <button type="button" className="save-button" disabled={!isReady} onClick={saveQuote}>{saved ? <Check size={17} /> : <Save size={17} />}<span>{saved ? "Salvo" : "Salvar"}</span></button>
            <button type="button" disabled={!isReady} onClick={copySummary} aria-label="Copiar resumo" title="Copiar resumo">{copied ? <Check size={17} /> : <Copy size={17} />}</button>
            <button type="button" disabled={!isReady} onClick={downloadSummary} aria-label="Baixar orçamento" title="Baixar orçamento"><Download size={17} /></button>
          </div>
        </aside>
      </div>
      ...
      {profileOpen ? (
        <div className="sheet-backdrop" role="presentation" onMouseDown={() => setProfileOpen(false)}>
          <section className="side-sheet profile-sheet" role="dialog" aria-modal="true" aria-label="Configurar minha hora" onMouseDown={(event) => event.stopPropagation()}>
            ...
            <p className="sheet-intro">Nada é preenchido por padrão. Defina sua referência e salve apenas se quiser reutilizá-la.</p>
            ...
            <div className="profile-preview"><span>hora-base calculada</span><strong>{result.baseHourlyRate > 0 ? currency.format(result.baseHourlyRate) : "—"}</strong></div>
                ...
                {history.map((item) => (
                  <article key={item.id} className="history-item">
                    <button type="button" className="history-load" onClick={() => loadSaved(item)}>
                      <span><strong>{item.name}</strong><small>{labels.projectType[item.input.projectType]} · {labels.pricingMode[item.pricingMode ?? "balanced"]} · {new Intl.DateTimeFormat("pt-BR", { dateStyle: "short" }).format(new Date(item.createdAt))}</small></span>
